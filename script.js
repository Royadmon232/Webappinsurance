...
(function() {
    ...
    function setupStreetAutocomplete(streets) {
        ...
        streetInput.addEventListener('input', inputHandler);
    }
...
(function() {
    ...
    function setupDynamicStreetDropdown() {
        ...
        async function handleCityChange() {
            const rawCityName = citySelect.value;
            // Use the existing helper function to handle name variations
            const selectedCity = normalizeCityName(rawCityName);
            ...
            if (streetCache.has(selectedCity)) {
                processStreets(streetCache.get(selectedCity), selectedCity);
                return;
            }
            ...
            try {
                ...
                processStreets(streets, selectedCity);
            } catch (error) {
                console.error('Error fetching streets:', error);
        ...
        function processStreets(streets, cityName) {
            if (streets.length > 0) {
                streetInput.disabled = false;
                streetInput.placeholder = 'הקלד שם רחוב לחיפוש';
                console.log(`Successfully loaded ${streets.length} streets for ${cityName}.`);
            } else {
                showError('לא נמצאו רחובות זמינים בעיר שבחרת, אנא נסה שוב מאוחר יותר.');
                console.warn(`No streets found for ${cityName}.`);
            }
        }
